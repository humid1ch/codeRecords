...
namespace July {
	...
	struct reverseIterator {
		...
		// 返回的是 当前位置正向迭代器的前一位的解引用，实质返回的是什么需要看
		// 正向迭代器的实现 为什么是正向迭代器的前一位？ 以 rbegin()
		// 获取的反向迭代器为例，获取的是最后一个元素的下一位，即不在容器的数据范围内，如果直接对当前位置的正向迭代器解引用，会发生错误，前一位才数据容器的数据范围
		Ref operator*() {
			Iterator tmp(_it);
	...
	public:
		...
		// 因为，int int
		// 是相同类型的传参，传参时编译器会调用最适合的传参类型，如果不重载一个传参类型
		// int, T的构造函数 则传参 int int 类型时，会发生不调用 用n个val (val
		// 是T类型的数据) 构造对象的构造函数
		...
		//  并将 this 与 tmp 用swap交换数据,
		//  由于只交换数据则需要自己实现swap，不能调用库swap
		...
		void reserve(size_t n) {
			...
			// vector的有效数据数量，计算应该是 _finish -
			// _start，但是开辟新空间之后，_finish 会变为 nullptr，无法相减 所以
			// vector的扩容之后，不能像 string一样直接首元素地址+_size 改变新对象的大小
			// 而是需要提前记录原对象数据大小，扩容之后，再使用 _start + sz 获得
			// _finish;
			size_t sz = size();
			if (_start + n > _endofstoage) {
				T* tmp = new T[n]; // 原对象内有数据
								   //// 就需要拷贝数据到新数据空间中，并且释放原对象数据空间
				if (_start)

				{
					// 这里用 memcpy 可能会发生深浅拷贝的问题，集中于
					// T为需要动态管理内存的类型时 建议画图、调试分析 所以不能用
					// memcpy直接拷贝内容，只能对单个空间数据，单独进行复制 赋值
					//
			...
			// 不能 _finish = _finish - _start,因为当前的_finish 和 _start
			// 不在同一片空间
			_finish = _start + sz;
		...
		// vector 中的 insert 与 string中的 insert参数不同，使用的是 iterator
		// 作为插入位置地址,且有返回值 也为 iterator 类型的 vector中的返回值是为了解决
		// 迭代器失效的问题 下面再考虑 insert的实现 需要移动数据，但是
		// iterator作为pos位置，就可以不用像string一样注意 size_t pos = -1的问题
