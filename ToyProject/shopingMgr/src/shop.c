...
Gift gifts[20] = {
    {"救赎", 1, 10}, {"日炎", 2, 10}, {"泰坦", 3, 10},
    {"水银", 4, 10}, {"偷偷", 5, 10}, {"反曲弓", 6, 10},
    {"青龙刀", 7, 10}, {"复活甲", 8, 10}, {"大天使", 9, 10},
    {"破甲弓", 10, 10}, {"心之钢", 11, 10}, {"羊刀", 12, 10},
    {"科技枪", 13, 10}, {"公理圆瓜", 14, 10}, {"金铲铲", 15, 10},
    {"巨杀", 16, 10}, {"红霸符", 17, 10}, {"鬼书", 18, 10},
    {"大法帽", 19, 10}, {"蓝霸符", 20, 10}
};
...
void showGifts() {
    ...
    for (int i = 0; i < 19; i += 2) {
        printf("礼品编号: %d\t礼品名: %s\t积分: %d ", gifts[i]._id, gifts[i]._name, gifts[i]._useInteg);
        printf("礼品编号: %d\t礼品名: %s\t积分: %d\n", gifts[i + 1]._id, gifts[i + 1]._name, gifts[i + 1]._useInteg);
    }
}
