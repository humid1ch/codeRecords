...
public:
	...
	static void* threadRoutine(void* args) {
		...
		int socketFd = tP->connectToServer();
		tP->_socks.push_back(socketFd);
		while (true) {
			// 即将通过任务队列给线程分配任务, 即 多线程访问临界资源, 需要上锁
			tP->lockQueue();
			while (!tP->haveTask() && tP->_running) {
	...
	// 建立连接, 可以在线程回调函数开始使用, 让线程维护一个长连接
	int connectToServer() {
		int sockfd = socket(AF_INET, SOCK_STREAM, 0);
		if (sockfd < 0) {
			LOG(FATAL, "threadPool thread create socket failed! errno: %d, desc: %s", errno, std::strerror(errno));
			exit(-3);
		}
		...
		if (connect(sockfd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
			LOG(FATAL, "threadPool thread connect server failed! errno: %d, strerror: %s", errno, std::strerror(errno));
			close(sockfd);
			exit(-4);
		}

		// 与服务端协商, 让其标记线程池线程的套接字, 单独处理
		Json::Value preAsk;
		preAsk["ask"] = "uploadFile";
		preAsk["pre"] = "yes"; // 表示 预先协商
		preAsk["owner"] = _owner.c_str();
		std::string preAskStr;
		jsonUtil::serialize(preAsk, &preAskStr);
		send(sockfd, preAskStr.c_str(), preAskStr.size(), 0);

		return sockfd;
	}

	void setOwner(const std::string& account) {
		_owner = account;
	}
...
private:
	...
	std::string _owner;		  // 记录线程池拥有者
	std::vector<int> _socks;  // 记录线程池创建的套接字
