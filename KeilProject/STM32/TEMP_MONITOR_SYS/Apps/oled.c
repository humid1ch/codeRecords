...
// 通信建立
// static void _OLED_writeDataBegin(void);
// 通信关闭
// static void _OLED_writeDataOver(void);

// 频繁建立 关闭通信的写数据函数(即每次发送数据 均新建立和关闭通信)
...
void OLED_showChinese(u8 x, u8 y, u8 font_w, const char* str) {
    ...
    // 记录要从哪个汉字编码数组中找
    const struct CnCharTypeDef* CnChar = (font_w == 16) ? CnChar16x16 : CnChar24x24;
