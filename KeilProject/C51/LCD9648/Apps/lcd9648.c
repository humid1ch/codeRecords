...
// font_wh: 表示汉字占用的像素宽度大小, 取值16, 24
void LCD9648_showChinese(u8 x, u8 y, u8 font_wh, const char* str) {
    u8 u8size = font_wh * font_wh / 8; // 汉字编码的字节数
    u8 i = 0;                          // 汉字编码遍历索引
    ...
    u8 num = (font_wh == 16) ? (sizeof(CnChar16x16) / sizeof(CnChar16x16[0])) : (sizeof(CnChar24x24) / sizeof(CnChar24x24[0]));
    // 记录要从哪个汉字编码数组中找
    struct CnCharTypeDef* CnChar = (font_wh == 16) ? CnChar16x16 : CnChar24x24;
    ...
    while (*cur != '\0') {
        if ((x > (LCD_WIDTH - font_wh)) || (y > (LCD_HEIGHT - 1 - 1)))
            break;
        ...
        for (j = 0; j < num; j++) {
            ...
            if ((CnChar[j].Index[0] == *cur) && (CnChar[j].Index[1] == *(cur + 1)) && (CnChar[j].Index[2] == *(cur + 2))) {
                ...
                x += font_wh; // 每显示一个汉字横索往向移动font_wh个字节
                break;
            }
        }
