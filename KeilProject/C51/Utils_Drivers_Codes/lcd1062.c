...
void LCD1602_Init() {
    ...
    _LCD1602_WriteData(MODE_SET | CURSOR_L | DISPLAY_STOP);
    delayMs();

    // 开关设置 显示器开 光标关 闪烁关
    _LCD1602_WriteData(SWITCH_SET | DISPLAY_ON | CURSOR_OFF | BLINK_OFF);
    delayMs();

    // 功能设置 数据总线宽度8 显示1行 高分辨率
    _LCD1602_WriteData(FUNC_SET | DATA_LINE_8BIT | ONE_LINE | CHAR_HIGH);
    delayMs();
}
